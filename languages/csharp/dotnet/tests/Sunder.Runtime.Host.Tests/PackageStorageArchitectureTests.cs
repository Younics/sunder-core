...
public sealed class PackageStorageArchitectureTests
{
    ...
    {
        var sdkRoot = Path.Combine(RepositoryRoot, "languages", "csharp", "dotnet", "packages", "Sunder.Sdk");
    ...
    {
        var source = ReadSources(Path.Combine(RepositoryRoot, "languages", "csharp", "dotnet", "host", "Sunder.App"));
    ...
    public void RuntimeHost_IsOnlyProductionOwnerCreatingPersistentPackageStorage()
    {
        ...
        {
            Path.Combine(RepositoryRoot, "languages", "csharp", "dotnet", "host"),
            Path.Combine(RepositoryRoot, "languages", "csharp", "dotnet", "packages"),
            Path.Combine(RepositoryRoot, "languages", "csharp", "dotnet", "libraries"),
        };
        var creators = productionRoots.SelectMany(root => Directory.EnumerateFiles(root, "*.cs", SearchOption.AllDirectories))
        ...
        Assert.All(creators, path => Assert.StartsWith(
            "languages/csharp/dotnet/host/Sunder.Runtime.Host/",
            path,
