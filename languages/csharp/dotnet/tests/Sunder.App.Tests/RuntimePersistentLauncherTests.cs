...
public sealed class RuntimePersistentLauncherTests
{
    ...
    public async Task MacSessionLauncher_BootstrapsPrivateCurrentSessionAgent()
    {
        ...
        try
        {
            var receipt = await launcher.ReconcileAndLaunchAsync(
                startInfo,
    ...
    public async Task MacSessionLauncher_WhenReplacementBootstrapFails_RestoresPreviousDefinition()
    {
        ...
        try
        {
            await Assert.ThrowsAsync<InvalidOperationException>(() => launcher.ReconcileAndLaunchAsync(
    ...
    public async Task MacSessionLauncher_WhenLoadedServiceHasNoDefinition_ReportsDisplacement()
    {
        ...
        try
        {
            await Assert.ThrowsAsync<HostServiceReplacementException>(() => launcher.ReconcileAndLaunchAsync(
                CreateMacStartInfo(root),
                replaceExisting: true,
                CancellationToken.None));
        }
    ...
    public async Task MacSessionLauncher_RealLaunchdReplacementRemainsObservable()
    {
        ...
        try
        {
            var first = await launcher.ReconcileAndLaunchAsync(startInfo, false, CancellationToken.None);
            var current = first;
            for (var attempt = 0; attempt < 10; attempt++)
            {
                current = await launcher.ReconcileAndLaunchAsync(startInfo, true, CancellationToken.None);
    ...
    public async Task MacSessionLauncher_ReplacementObservesBootoutBeforeBootstrap()
    {
        ...
        try
        {
            await launcher.ReconcileAndLaunchAsync(
                CreateMacStartInfo(root),
                replaceExisting: true,
                CancellationToken.None);
    ...
    public async Task LinuxSessionLauncher_WhenReplacementLaunchFails_ReportsDisplacedService()
    {
        ...
        var exception = await Assert.ThrowsAsync<HostServiceReplacementException>(() =>
            launcher.ReconcileAndLaunchAsync(
    ...
    public async Task LinuxSessionLauncher_RejectsPersistentUnitNameCollision()
    {
        ...
        var exception = await Assert.ThrowsAsync<InvalidOperationException>(() => launcher.ReconcileAndLaunchAsync(
            CreateLinuxStartInfo(),
            replaceExisting: true,
            CancellationToken.None));
    ...
    public async Task WindowsLauncher_ObservesExactProcessExit()
    {
        ...
        try
        {
            var receipt = await launcher.ReconcileAndLaunchAsync(startInfo, false, CancellationToken.None);
    ...
    public async Task LinuxDetachedFallback_ReconcilesAcrossLauncherInstances()
    {
        ...
        try
        {
            var first = await firstLauncher.ReconcileAndLaunchAsync(startInfo, false, CancellationToken.None);
            firstLauncher.Dispose();
            var adopted = await secondLauncher.ReconcileAndLaunchAsync(startInfo, false, CancellationToken.None);
            replacement = await secondLauncher.ReconcileAndLaunchAsync(startInfo, true, CancellationToken.None);
