    ...
    PackageSessionState? packageSessionState = null)
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            if (target.Kind is SunderPackageFormat.WorkerTargetKind or SunderPackageFormat.ProcessTargetKind)
            {
                if (rpcBroker is null)
                {
                    throw new InvalidOperationException(
                        $"Package '{package.PackageId}' worker Runtime target requires the Runtime RPC broker.");
                }
                ...
                var workerProtocol = target.RequiredHostCapabilities?.Contains(
                    SunderWorkerProtocol.V2Capability,
                    StringComparer.Ordinal) == true
                    ? SunderWorkerProtocol.V2
                    : SunderWorkerProtocol.V1;
                if (workerProtocol.UsesV2Lifecycle && packageSessionState is null)
                {
                    throw new InvalidOperationException(
                        $"Package '{package.PackageId}' V2 worker Runtime target requires active-session authority.");
                }
                ...
                processServices.AddSingleton<IPackageContext>(processPackageContext);
                ...
                if (workerProtocol.UsesV2Lifecycle)
                {
                    await worker.ComposeAsync(cancellationToken);
                }
                var processSettingsSchema = worker.SettingsSchema;
                processPackageContext.PackageSettings.Schema = processSettingsSchema;
                ...
                var processLoadedPackage = new ActiveLoadedPackage(
                    ...
                    package.Source,
                    ToProtocolSettingsSchema(
                        package.PackageId,
                        package.Activation.Name,
                    ...
                    processPackageContext.Settings)
                {
                    ...
                    CanonicalSettingsSchema = processSettingsSchema,
                ...
                {
                    warnings.Add($"Package '{package.PackageId}' worker Runtime target loaded without any declared RPC providers.");
                }
