...
internal static class PackageLogFormatter
{
    ...
    private static readonly HashSet<string> ReservedFieldNames = new(StringComparer.Ordinal)
    {
        "category",
        "package_version",
        "source",
        ...
        string? eventName)
    {
        ...
        foreach (var item in attributes)
        {
            ...
            var fieldName = SanitizeFieldName(item.Key);
            if (ReservedFieldNames.Contains(fieldName)) fieldName = "attribute_" + fieldName;
            sanitized[fieldName] = IsSensitiveKey(item.Key) ? "[REDACTED]" : SanitizeValue(item.Value);
        }
    ...
    {
        fields[$"{prefix}_type"] = exception is RemotePackageLogException remote
            ? remote.RemoteType
            : exception.GetType().FullName;
        fields[$"{prefix}_message"] = exception.Message;
        if (exception is not RemotePackageLogException)
        {
            fields[$"{prefix}_hresult"] = exception.HResult;
        }
        fields[$"{prefix}_stacktrace"] = exception is RemotePackageLogException remoteStack
            ? remoteStack.RemoteStackTrace
...
internal sealed class RemotePackageLogException(
    string remoteType,
    string message,
