...
internal static class RuntimeRpcEndpoints
{
    ...
    public static IEndpointRouteBuilder MapRuntimeRpcEndpoints(this IEndpointRouteBuilder endpoints)
    {
        ...
        rpc.MapPost("app/provider", GetAppProviderAsync)
            .WithMetadata(new RequestSizeLimitAttribute(MaxRequestBodyBytes));
        rpc.MapPost("app/invariant-violation", ReportAppInvariantViolationAsync)
            .WithMetadata(new RequestSizeLimitAttribute(MaxRequestBodyBytes));
    ...
    private static async Task<IResult> ReportAppInvariantViolationAsync(
        RuntimeRpcAppInvariantViolationRequest request,
        HttpContext context,
        CancellationToken cancellationToken)
    {
        var broker = context.RequestServices.GetRequiredService<RuntimeRpcBroker>();
        try
        {
            var accepted = request.CallScopeId is null
                ? await broker.TryReportInvariantViolationAppAsync(
                    request.SessionId,
                    Endpoint(request.EndpointReference),
                    request.ExceptionMessage,
                    cancellationToken).ConfigureAwait(false)
                : await broker.TryReportInvariantViolationScopeAsync(
                    broker.GetAppCallScope(request.SessionId, request.CallScopeId),
                    Endpoint(request.EndpointReference),
                    new InvalidOperationException(request.ExceptionMessage),
                    cancellationToken).ConfigureAwait(false);
            return Results.Ok(new RuntimeRpcAppInvariantViolationResponse(accepted, null));
        }
        catch (SunderRpcException exception)
        {
            return Results.Ok(new RuntimeRpcAppInvariantViolationResponse(false, ToProtocol(exception.Error)));
        }
    }
