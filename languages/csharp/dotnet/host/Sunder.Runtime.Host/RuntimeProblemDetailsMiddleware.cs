    ...
    ILogger<RuntimeProblemDetailsMiddleware> logger)
{
    ...
    public async Task InvokeAsync(HttpContext context)
    {
        ...
        catch (Exception exception) when (IsHostAuthenticatedRpcCancellation(exception))
        {
    ...
    private static bool IsHostAuthenticatedRpcCancellation(Exception exception)
    {
        var foundRpcCancellation = false;
        var pending = new Stack<Exception>();
        pending.Push(exception);
        while (pending.TryPop(out var current))
        {
            if (current is SunderRpcException rpcException)
            {
                if (!rpcException.IsHostAuthenticated
                    || rpcException.Error.Kind != SunderRpcErrorKind.Cancelled)
                {
                    return false;
                }
                foundRpcCancellation = true;
                continue;
            }
