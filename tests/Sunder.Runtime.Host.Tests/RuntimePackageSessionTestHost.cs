...
internal sealed class RuntimePackageSessionTestHost
{
        ...
        RuntimeEventStreamService? eventStream = null)
    {
        ...
        var publisher = new PackageSessionPublisher(_owner, NullLogger<PackageSessionPublisher>.Instance);
    ...
    {
        var result = await _storeCoordinator.ExecuteAsync(
            [new PackageStoreMutation(PackageStoreMutationKind.Upgrade, packageId, path, Reinstall: true)],
            token);
        return result.Success
            ? await _installed.ReloadAsync(new InstalledPackageSessionReloadRequest(result.ImpactedPackageIds), token)
            : result;
    }
    ...
    public IReadOnlyList<PackageConfigurationSchemaDescriptor> GetConfigurationSchemas() => _settings.GetSchemas();
