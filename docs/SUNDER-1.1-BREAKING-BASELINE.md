...
- Package configuration and session abstractions were replaced by settings, role-local workspace, callbacks, and typed Runtime operations.
...
- App presentation faults are client-local; the obsolete App-to-Runtime fault-report endpoint and DTO were removed.
...
- Public Sunder-to-Sunder NuGet dependencies use `[1.1.0,1.2.0)`, and generated runtime package dependencies default to `>=1.1.0 <1.2.0`.
- Public SDK assemblies keep binary `AssemblyVersion` `1.1.0.0` throughout the compatible 1.1.x line; package and informational versions continue to identify each release.
...
- V1 package builds always emit SDK API version `1`; the unused fixed API-version and diagnostic SDK-version overrides were removed.
...
The stable lower bound deliberately excludes `1.1.0-*`. SDK and Agent release workflows reject initial-baseline prerelease tags rather than publishing artifacts that the Host, templates, or Agent dependency graph cannot consume.
