...
internal sealed class PackageSessionPreparer
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        if ((hostRoles & PackageHostRoles.Runtime) != 0)
        {
            ...
            if (exactRuntimeTarget.Kind is not (SunderPackageFormat.DotnetTargetKind or SunderPackageFormat.ProcessTargetKind))
            {
                errors.Add(
                    $"Package '{manifest.Id}' exact Runtime target '{runtimeTargetKey}' has unsupported target kind '{exactRuntimeTarget.Kind}'. The Runtime Host supports '{SunderPackageFormat.DotnetTargetKind}' and '{SunderPackageFormat.ProcessTargetKind}' targets.");
                return null;
            }
            ...
            if (string.Equals(exactRuntimeTarget.Kind, SunderPackageFormat.ProcessTargetKind, StringComparison.Ordinal)
            ...
            if (sourceKind == PackageSourceKind.Dev
                && string.Equals(exactRuntimeTarget.Kind, SunderPackageFormat.ProcessTargetKind, StringComparison.Ordinal))
            {
