...
// Compatibility facade for in-process SDK/tests. HTTP endpoints use the focused services directly.
internal sealed class RuntimePackageSessionService
{
    ...
    private readonly PackageConfigurationAccessService _configuration;
        ...
        RuntimeEventStreamService? eventStream = null)
    {
        ...
        _owner = new RuntimeSessionOwner(NullLogger<RuntimeSessionOwner>.Instance, events);
        _ui = new RuntimePackageUiService(_owner, snapshots, installedPackageStore);
        var reconciler = new PackageSessionReconciler(logger, installedPackageStore, packageArchiveInstaller.Paths);
        ...
        _configuration = new PackageConfigurationAccessService(_owner);
    ...
    internal Task<IReadOnlyList<DevPackageWatchTarget>> ConfigureDevPackageWatchingAsync(bool enabled, CancellationToken token = default) => _sessions.ConfigureDevWatchingAsync(enabled, token);
    ...
    public Task<PackageLifecycleOperationResult> LoadPackageLifecycleAsync(PackageLifecycleLoadRequest request, CancellationToken token = default) => _sessions.LoadPackageLifecycleAsync(request, token);
    ...
    public IReadOnlyList<PackageConfigurationSchemaDescriptor> GetConfigurationSchemas() => _configuration.GetSchemas();
    public Task<PackageConfigurationValuesResponse?> GetConfigurationValuesAsync(string packageId, CancellationToken token = default) => _configuration.GetValuesAsync(packageId, token);
    public Task<bool> SaveConfigurationValuesAsync(string packageId, UpdatePackageConfigurationValuesRequest request, CancellationToken token = default) => _configuration.SaveValuesAsync(packageId, request, token);
    ...
    public Task<PackageAuthSessionStartResponse?> StartPackageAuthAsync(string packageId, PackageAuthCallbackServer server, CancellationToken token = default) => _auth.StartAsync(packageId, server, token);
    ...
    public Task<bool> CompletePackageAuthSessionAsync(string sessionId, IReadOnlyDictionary<string, string?> values, CancellationToken token = default) => _auth.CompleteAsync(sessionId, values, token);
    ...
    public Task<PackageOperationResult> ReloadInstalledPackageSessionAsync(InstalledPackageSessionReloadRequest request, CancellationToken token = default) => _installed.ReloadAsync(request, token);
    ...
    public Task ShutdownAsync() => _installed.ShutdownAsync(_sessions);
    ...
    public Task<PackageDataValueResponse?> GetPackageConfigurationAsync(string packageId, string key, CancellationToken token = default) => _data.GetStateAsync(packageId, key, token);
    ...
    private async Task<PackageSessionOperationResult> ToSessionResultAsync(PackageOperationResult result, string packageId, CancellationToken token)
        => new(result.Success, result.Message, result.Warnings, result.Errors, result.ImpactedPackageIds.Count == 0 ? [packageId] : result.ImpactedPackageIds, await _sessions.GetStatusAsync(packageId, token));
}
